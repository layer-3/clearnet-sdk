...
import (
	...
	sharedeip712 "github.com/layer-3/clearnet-sdk/pkg/eip712"
)
...
func uint256Word(field string, n *big.Int) []byte {
	if n == nil || n.Sign() < 0 || n.BitLen() > 256 {
		panic(fmt.Sprintf("evm: invalid EIP-712 uint256 %s: %v", field, n))
	}
...
func domainSeparator(name string, chainID uint64, contract common.Address) common.Hash {
	return sharedeip712.ComputeDomainSeparatorFor(name, EIP712Version, new(big.Int).SetUint64(chainID), contract)
}
func typedDigest(name string, chainID uint64, contract common.Address, structure common.Hash) common.Hash {
	...
	return common.BytesToHash(sharedeip712.Digest(domain, structure))
}
...
func ComputeWithdrawalDigest(chainID uint64, vault, to, asset common.Address, amount *big.Int, withdrawalID [32]byte, deadline *big.Int) common.Hash {
	return typedDigest(CustodyDomainName, chainID, vault, typedStructHash(executeType, addressWord(to), addressWord(asset), uint256Word("amount", amount), withdrawalID[:], uint256Word("deadline", deadline)))
}
