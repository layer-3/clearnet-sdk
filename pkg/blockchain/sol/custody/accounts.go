...
func ParseAnyAccount(accountData []byte) (any, error) {
	...
	case Account_Config:
		...
		return value, nil
	case Account_WithdrawalRecord:
		value := new(WithdrawalRecord)
		err := value.UnmarshalWithDecoder(decoder)
		if err != nil {
			return nil, fmt.Errorf("failed to unmarshal account as WithdrawalRecord: %w", err)
		}
		return value, nil
	default:
...
func ParseAccount_WithdrawalRecord(accountData []byte) (*WithdrawalRecord, error) {
	...
	if discriminator != Account_WithdrawalRecord {
		return nil, fmt.Errorf("expected discriminator %v, got %s", Account_WithdrawalRecord, binary.FormatDiscriminator(discriminator))
	}
	acc := new(WithdrawalRecord)
	err = acc.UnmarshalWithDecoder(decoder)
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal account of type WithdrawalRecord: %w", err)
	}
	return acc, nil
}
