...
func TestNewFromBigIntCopiesInput(t *testing.T) {
	input := big.NewInt(12345)
	d := NewFromBigInt(input, -2)

	input.SetInt64(0)

	if got, want := d.String(), "123.45"; got != want {
		t.Fatalf("NewFromBigInt retained mutable input pointer: got %s, want %s", got, want)
	}
}
...
func TestNewFromBigRatCopiesInput(t *testing.T) {
	input := big.NewRat(2, 3)
	d := NewFromBigRat(input, 4)

	input.SetInt64(0)

	if got, want := d.String(), "0.6667"; got != want {
		t.Fatalf("NewFromBigRat retained mutable input pointer: got %s, want %s", got, want)
	}
}
...
func TestBigIntReturnsCopy(t *testing.T) {
	...
	if d.IntPart() != 123 {
		t.Fatalf("mutating BigInt result mutated Decimal: got %s", d.String())
	}
}
...
func TestFromWei(t *testing.T) {
	for _, tc := range []struct {
		...
		{"123456789000000000000000000", "123456789"},
		...
		if d.String() != tc.expected {
			t.Errorf("FromWei(%s): expected %s, got %s", tc.wei, tc.expected, d.String())
		}
	}
}

func TestDecimal_ToWei(t *testing.T) {
	for _, tc := range []struct {
		dec      string
		expected string
	}{
		...
		{"123456789", "123456789000000000000000000"},
		{"-2.5", "-2500000000000000000"},
		// sub-wei fractions are truncated
		{"0.0000000000000000001", "0"},
		{"1.0000000000000000005", "1000000000000000000"},
	} {
		d := RequireFromString(tc.dec)
		got := d.ToWei()
		...
		if got.Cmp(expected) != 0 {
			t.Errorf("(%s).ToWei(): expected %s, got %s", tc.dec, tc.expected, got.String())
		}
	}
}

func TestFromWei_ToWei_Roundtrip(t *testing.T) {
	original := new(big.Int)
	original.SetString("12345678901234567890", 10)
	d := FromWei(original)
	back := d.ToWei()
	if back.Cmp(original) != 0 {
		t.Errorf("roundtrip failed: started with %s, got back %s (decimal was %s)",
			original.String(), back.String(), d.String())
	}
}

func TestExpTaylor_Concurrent(t *testing.T) {
	// Verify no data race when calling ExpTaylor concurrently.
