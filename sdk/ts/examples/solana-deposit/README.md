...
| Asset | blank | Empty string native SOL sentinel. Use an SPL mint address for token deposits. |
| Reference | blank | Optional `bytes32` deposit reference. |
| Amount | `0.1` | Decimal token amount. Native SOL uses 9 decimals; SPL tokens use mint decimals. |
...
| SPL deposit fails with an account error | The SPL depositor ATA or vault ATA does not exist, or the depositor ATA has no token balance. | Create and fund both token accounts before submitting, or leave `Asset` blank for the native browser smoke test. |
